...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function(exports){
    ...
    var $ = layui.$
        ...
        //外部接口
        ,table = {
            config: {//Global configuration item,Table level
                indexName: 'lay_table_index' //Bidding index name
                ,cols:{//节点级别的附加字段
                    isCheckName: 'lay_is_checked' //Selected status (True, FALSE)
                    ,isRadio:'lay_is_radio'//True status (True, FALSE)
                    ,isOpen:'lay_is_open'//Whether to expand nodes
                    ,isShow:'lay_is_show'//Whether to display nodes
                    ,level:'lay_level'//The hierarchical relationship of nodes (no need to set)
                    ,children:'children'//Store the variables of the lower level

                    ,cheDisabled:'lay_che_disabled'//No choice (True, FALSE)
                    ,radDisabled:'lay_rad_disabled'//Prohibited alone (True, FALSE)

                    ,iconOpen:'lay_icon_open'//Open icon
                    ,iconClose:'lay_icon_close'//Close icon
                    ,icon:'lay_icon'//Leaf node icon
                }
                ,initWidth:{//Definition of default column width
                    checkbox: 48
                    ,space: 15
                    ,numbers: 40
                    ,radio:48
             ...
             * Structural icon
             * cache{}                  Cache (object)
             *      key['data']{}       All data cache (object)
             *          key[list][]     List data object (array)
             *          key[map]{}      List data MAP object (MAP)
             *          key[treeList][] Object (array) of the tree structure (array)
             *      key['cla']{}        All CALSS objects that have been initialized (note that rendering is asynchronous execution)
             *          key['claIds'][] All table classes that have been hung up with initialization methods
             *          key[claObjs]{key[tableId]}  All CLA objects that have been initialized
             *
             */
            ,cache: {
                tableId:{
                    data:{
                        list:[]//List data
                        ,map:{}//List data is based on IDField or unique value as Key's MAP data
                        ,treeList:[]//Tree data
                        ,upIds:[]//Father node collection  在一次外部请求前严格按照第一次的顺序
                    }
                }
                ,cla:{
                    claIds:{
            ...
            ,init:function (filter, settings) {
                ...
                elemTable.each(function(){
                    ...
                    var cols = [], options = $.extend({
                        ...
                        ,cols: []
                        ,data: []
                        ,skin: othis.attr('lay-skin') //style
                        ,size: othis.attr('lay-size') //size
                        ,even: typeof othis.attr('lay-even') === 'string' //Puppet background
                    ...
                    othis.find('thead>tr').each(function(i){
                        ...
                        $(this).children().each(function(ii){
                            ...
                            var row = $.extend({
                                title: th.text()
                                ,colspan: th.attr('colspan') || 0 //Column cell
                                ,rowspan: th.attr('rowspan') || 0 //Line
                            }, itemData);
            ...
            /**
             * Rendering entry method (core entrance)
             */
            ...
            /**
             * The corresponding table is loaded and executed after completion(Methods have been abandoned, please use PARSEDATA instead)
             * @param tableId
             * @param fn
             */
                ...
                function isReady() {
                    if(tableId){
                        ...
                        if(that&&that.hasOwnProperty('layBody')){
                            ...
                            if(i<=(1000*10)&&!is){//Exit greater than 10 seconds
            ...
            ,checkStatus:function (tableId) {
                ...
                layui.each(data, function(i, item){
                    if(item.constructor === Array){
                        invalidNum++; //Effect data, or deleted
                        return;
                    }
                ...
                return {
                    ...
                    ,isAll: data.length ? (nums === (data.length - invalidNum)) : false //Whether to choose
                };
            }
            /**
             * Set the form check status
             * @param tableId
             * @param value     When this value exists, it is set to operate
             * @returns {*}
             */
            ...
            /**
             * Set the form single selection status
             * @param tableId
             * @param value     When this value exists, it is set to operate
             * @returns {*}
             */
            ,setRadioStatus:function (tableId,fildName,value) {
                ...
                if(fildName&&value){//Set selection
                    data.forEach(function (e) {
                        var b1=e[fildName]+"";
                        var b2=value+"";
                        if(b1==b2){
            ...
            /**
             * This method is re -rendering as a whole (heavyweight refresh method)
             * @param id
             * @param options
             */
            ...
            /**
             * Add a row or multi -line data
             * @param tableId   Table ID
             * @param index     Insert in the first few positions (starting from 0)
             * @param data      data
             * @returns {*}
             */
            ,addRow:function (tableId, index, data) {
                var that=table.getClass(tableId)
                    ,options=that.config
                    ,uo = []//Parent -level node
                    ,treeList=table.getDataTreeList(tableId)
                    ,list = table.getDataList(tableId) || [];
                that.resetData(data);
                //Insert behind the parent node
                list.splice(index,0,data);//更新缓存
                table.kit.restNumbers(list);//Reset the bidding
                table.setDataMap(tableId,that.resetDataMap(list));//Process MAP
                if(options.isTree){//Processing level
                    //1. Proce2. Treatment TreeObjt -level  2. Treatment TreeObj 3. level
                ...
                if(index==0){//Insert at the first position
                    var  tbody=that.layBody.find('table tbody');
                    $(tbody).prepend(trs);
                    that.layBody.find(".layui-none").remove();
                }else{
                    var o=that.layBody.find('[data-index='+(index-1)+']');//Father node DOM tree
                    $(o).after(trs);
                }
                that.renderForm();
                if(options.isPage)that.renderPage(that.config.page.count+1);//Paging rendering
                that.restNumbers();
                that.events();
                if(options.isTree) {//Unfolding node
                    that.treeNodeOpen(uo, true);
                    that.renderTreeConvertShowName(uo);
                }
            }
            /**
             * Delete one line or multi -line data
             * (If it is tree -shaped, delete yourself and sub -nodes)
             * @param tableId
             * @param data（1、ArrayObject、对象）
             */
            ,delRow:function (tableId, data) {
                //1. Page clearance 2. Cchecge
                var that=table.getClass(tableId)
                    ,options=that.config
                    ,list=table.getDataList(tableId);
                var sonList=[];//Data needed to be deleted
                var delIds={};//Data Map to be deleted
                var delDatas=[];
                var upDelDatas=[];//All the parent nodes of the node to be deleted (processed folding)
                if(!that||!data)return;
                if(table.kit.isArray(data)){//Is an array, delete multiple
                    delDatas=data;
                }else{
                    delDatas[0]=data;
                }
                ...
                sonList.forEach(function (temo) {//Page element processing
                    var index=temo[table.config.indexName];
                    delIds[index]=index;//Set the ID collection deleted on behalf of the delete
                    var tr = that.layBody.find('tr[data-index="'+ index +'"]');
                    tr.remove();
                });
                that.restNumbers();//data processing
                var newList=[];//Reconstruct a new array
                ...
                table.kit.restNumbers(newList);//Number re -number
                table.setDataList(tableId,newList);//Treatment list
