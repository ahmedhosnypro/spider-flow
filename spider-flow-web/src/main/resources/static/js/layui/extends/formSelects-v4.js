...
/**
 * name: formSelects
 * Based onSelect multiple choiceSelect multiple choices
 * version: 4.0.0.0910
...
(function (layui, window, factory) {
	...
	var v = '4.0.0.0910',
	    ...
	    quickBtns = [{ icon: 'xm-iconfont icon-quanxuan', name: 'select all', click: function click(id, cm) {
			cm.selectAll(id, true, true);
		} }, { icon: 'xm-iconfont icon-qingkong', name: 'Empty', click: function click(id, cm) {
			cm.removeAll(id, true, true);
		} }, { icon: 'xm-iconfont icon-fanxuan', name: '反选', click: function click(id, cm) {
			cm.reverse(id, true, true);
		} }, { icon: 'xm-iconfont icon-pifu', name: 'Skin exchange', click: function click(id, cm) {
			cm.skin(id);
		} }],
		...
		this.config = {
			...
			init: null, //The selection value of initialization,
			on: null, //Select value changes
			opened: null,
			closed: null,
	...
	//Some simple processing methods
	...
	Common.prototype.appender = function () {
		//Some expansion for IE
		//Expand Array MAP method
	...
	Common.prototype.init = function (target) {
		...
		//SELECT on the initialized page
		$(target ? target : 'select[' + NAME + ']').each(function (index, select) {
			...
			//First cancel the rendering of Layui on SELECT
			hasLayuiRender[0] && hasLayuiRender.remove();
			hasRender[0] && hasRender.remove();

			//Constructed rendering div
			...
			//If you Plus incidentsearch, 加上事件
	...
	Common.prototype.search = function (id, e, searchUrl, call) {
		...
		if (searchUrl) {
			...
			//Traversal option, Select the value that can be displayed
	...
	Common.prototype.ajax = function (id, searchUrl, inputValue, isLinkage, linkageWidth, isSearch, successCallback, isReplace) {
		...
		//Do you need to add random time to AJAX
		//ajaxData['_'] = Date.now();
		$.ajax({
	...
	Common.prototype.renderData = function (id, dataArr, linkage, linkageWidth, isSearch, isReplace) {
		...
		if (isSearch) {
			//If it is Here we need to be judged search, 这里需要判重
	...
	Common.prototype.renderLinkage = function (id, dataArr, linkageWidth) {
		...
		reElem.find('.' + INPUT).css('display', 'none'); //Links do not support search temporarily
	};
	...
	Common.prototype.one = function (target) {
		...
		$(target ? target : document).off('click', '.' + FORM_TITLE).on('click', '.' + FORM_TITLE, function (e) {
			...
			//Clear non -Ben SELECT input input val
			...
			if (othis.is('i[fsw="' + NAME + '"]')) {
				...
				if (dd.hasClass(DISABLED)) {
					//If it is DisablUnsatNon -deletedactorystatus, 不可选, 不可删
					return false;
				}
		...
		$(target ? target : document).off('click', 'dl.' + DL).on('click', 'dl.' + DL, function (e) {
			...
			if (othis.is('.' + LINKAGE) || othis.parents('.' + LINKAGE)[0]) {
				...
				//Activate the next group, Display corresponding data before activation
				...
				//If there is noOr there is no corresponding valueroup, 或没有对应的值
			...
			if (dd.hasClass(DISABLED)) {
				//Treatment of the disabled options
				return false;
			}
	...
	Common.prototype.handlerLabel = function (id, dd, isAdd, oval, notOn) {
		...
		if (!div[0]) return;
		//Close the selection domain after the single selection
		if (fs.config.radio) {
			this.changeShow(div, false);
		}
		//Remove the red bezel verified by the form
	...
	Common.prototype.retop = function (div) {
		//Calculate the position of DL display
		var dl = div.find('dl'),
	...
	Common.prototype.changeShow = function (children, isShow) {
		...
		if (isShow) {
			...
			//Calculate whether Ajax data is empty, Then re -request data
	...
	Common.prototype.changePlaceHolder = function (div) {
		//Show in hidden tips
		//Adjust the height in the PANE mode
		...
		if (data[id] && data[id].config.height) {//Since it Then look at ited, 那就看着办吧

		} else {
			var height = title.find('.' + NAME)[0].clientHeight;
			title.css('height', (height > 36 ? height + 4 : height) + 'px');
			//If it is LAYUI PANE mode, Treatment of label height
	...
	Common.prototype.removeAll = function (id, isOn, skipDis) {
		...
		if (dl.find('.xm-select-linkage')[0]) {
			//Treatment for multi -level linkage
	...
	Common.prototype.onreset = function () {
		//SurveillancThen reset multiple choicest button, 然后重置多选
