...
$(function(){
	...
	if (!mxClient.isBrowserSupported()){
		...
		$("body").on("mousewheel",".layui-tab .layui-tab-title",function(e,delta){
			...
			layui.layer.open({
				type : 1,
				title : 'Please enter cookie',
				content : `<textarea id="cookies" name="cookies" placeholder="Please enter the cookies, the segment number( ; )Separate cookie, equal to the number( = )Separate the name and value" autocomplete="off" class="layui-textarea"  lay-verify="required" style="height:250px"></textarea>`,
				area : '800px',
				btn : ['closuset up','设置'],
				btn2 : function(){
					...
					for (var i = 0; i < cookieArr.length; i++) {
						...
						if (index < 0) {
							layer.alert('cookie data format error');
							appendFlag = false;
							return;
						} else {
							data.push({
			...
			layui.layer.open({
				type : 1,
				title : 'Please enter the header',
				content : `<textarea id="headers" name="headers" placeholder="Please enter the headerDivide the name and valuee, a fake number( : )分割name和value" autocomplete="off" class="layui-textarea"  lay-verify="required" style="height:250px"></textarea>`,
				area : '800px',
				btn : ['closure','set up'],
				btn2 : function(){
			...
			layui.layer.open({
				type : 1,
				title : 'Please enter the parameter',
				content : `<textarea id="paramters" name="paramters" placeholder="Please enter the paramete), Space (space (spacee by one, a colon( : ),equal sign( = ), Space (space (space  ）或tab（ \t ) Divide the name and value" autocomplete="off" class="layui-textarea"  lay-verify="required" style="height:250px"></textarea>`,
				area : '800px',
				btn : ['closuset up','设置'],
				btn2 : function(){
					...
					for (var i = 0; i < paramterArr.length; i++) {
						...
						if (index < 0) {
							layer.alert('Parameter data format error');
							return;
						} else {
							data.push({
	...
	function loadShapes(editor,container){
		...
		var shapes = [{
			...
			title : 'Start grab',
			desc : 'Grab the static HTML page or API inteThe support method reference command prompt.the result is stored as a resp variable.<br/>支持方法参考命令提示。'
		},{
			name : 'variable',
			...
			desc : 'Define the pDefinition variables have the order of order. First, define variables can be used in the future, and drag can exchange variable order.定义变量后续可以使用，拖动可以交换变量顺序。'
		},{
			name : 'loop',
			...
			title : 'Output',
			desc : 'Variable results in the output process (only use only) is useful)'
		},{
			name : 'executeSql',
			...
			title : 'Execute SQL',
			desc : 'SQL is executed, and the data souThe statement type is: select, return configured. The SQL execution results are stored in variable RS.<br/>The statement type is: select, return:List&lt;Map&lt;String,Object&gt;&gt;<br/>The statement type is: selectone, return:Map&lt;String,Object&gt;<br/>The statement type is: selectint, return:Integer<br/>The statement type is: insert,int, batch operation returns int array, return:int, batch operation returns int array<br/>Variables in SQL must be used # # Package, such as:#${title}#'
		},{
			name : 'function',
			...
			desc : 'Execute the function method alone, and the result is not saved to variable'
		},{
			name : 'process',
			...
			title : 'Sub -process',
			desc : 'Execute other SpiderFlow processes, the father -son process variable sharing'
		}];
...
function runSpider(debug){
	validXML(function(){
		...
		var testWindowIndex = layui.layer.open({
			...
			maxHeight : 400,
			title : 'Test window',
			btn : ['closure','Display/hiShow/hidden logtput','Show/hidden log','stop'],
			btn2 : function(){
...
//Recently click on the pop -up window
var index;
...
function createWebSocket(options){
	...
	socket.onerror = options.onerror || function(){
		layer.layer.msg('Websocket error');
	}
	return socket;
}
	...
	validXML(function(){
		$.ajax({
			...
			data : {
				...
				xml : editor.getXML(),
				name : editor.graph.getModel().getRoot().data.get('spiderName') || 'Unfalted name',
			},
			success : function(id) {
				flowId = id;
